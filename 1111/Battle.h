...
private:
    void displayBattleStatus() const {
        ...
        // 玩家名称和等级（左侧）
        cout << "  ";
        Colors::printCyan(player->getName());
        ...
        // 玩家攻击力和防御力（左侧）
        cout << "  攻击: ";
        Colors::printYellow(to_string(player->getAttack()));
        cout << " 防御: ";
        Colors::printCyan(to_string(player->getDefense()));
    ...
    bool handleAttack() {
        // 新伤害公式：最终 = floor(基础 * (1 - min(0.5, DEF*0.04)))，至少为1
        int base = player->getAttack();
        double dr = std::min(0.5, enemy->getDefense() * 0.04);
        int damage = std::max(1, (int)std::floor(base * (1.0 - dr)));
