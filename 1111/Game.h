...
private:
    ...
    bool startNewGame() {
        ...
        gameMap = new Map();
        // 根据 partC(2).docx 初始化玩家基础数值
        player->loadFromData(1, 120, 120, 20, 6, 0, 100);
        
        clearScreen();
        Colors::printSuccess("欢迎你, " + playerName + "！");
        showPrologue(playerName);
        Colors::printSuccess("整装待发，冒险开始！");
        
        return true;
    }

    // 序章剧情
    void showPrologue(const string& name) {
        Colors::printTitle("序章 · 山海旧梦");
        Colors::printInfo("夜色沉入龙渊，崂山的风吹动松针，发出细碎的呢喃。");
        Colors::printInfo("你——" + name + "，背着略显陈旧的行囊，沿着石阶一步步登临。");
        Colors::printGray("传说里，龙王沉睡千年，凡有心者可唤其醒；但醒来的不止是龙王，还有人心深处未解的迷与执。");
        Colors::printGray("山道尽头的古庙前，一枚风铃无风自鸣，你在铜镜中看到自己的影子，像被时间反复打磨过的石器。");
        Colors::printInfo("‘若能越过迷雾，便能听见深渊回响。’你想起师门的叮嘱，握紧了掌心的符箓。");
        Colors::printInfo("你决定踏入山门——");
        Colors::printSubSeparator(60);
        waitForInput();
        
        Colors::printTitle("序章 · 迷雾初开");
        Colors::printInfo("古道两旁的石兽覆满青苔，脚下的路延伸向看不见的暗处。");
        Colors::printInfo("你感觉到某种目光在暗处游移，那不是敌意，像是在等待你的回答。");
        Colors::printGray("风里带着海盐的味道——崂山的海，总在最远的地方回应登山的人。");
        Colors::printInfo("你深吸一口气，向前迈出第一步。");
        Colors::printSubSeparator(60);
        waitForInput();
    }

    // 终章剧情
    void showEpilogue(const string& name) {
        Colors::printTitle("终章 · 龙渊将息");
        Colors::printInfo("龙王化作万千霞光，散入山海之间。殿宇微颤，风铃止息。");
        Colors::printInfo("你——" + name + "，立于石阶之上，忽闻远处海潮，如心跳归于平缓。");
        Colors::printGray("你想起初来之夜，那句未竟的问题：‘唤醒的究竟是龙王，还是自己？’");
        Colors::printGray("此刻答案不再重要。你转身，身后雾散，山路一层层亮了起来。");
        Colors::printSubSeparator(60);
        waitForInput();
        
        Colors::printTitle("终章 · 去留");
        Colors::printInfo("古庙门前，铜镜里的影子与真实重合，你在镜面上按下一枚掌心的温度。");
        Colors::printInfo("风铃终于在风里作响，像是谁在道一声‘辛苦了’。");
        Colors::printSuccess("你收起符箓，目光温和而坚定。崂山的故事，至此告一段落——");
        Colors::printInfo("而你的故事，仍在继续。");
        Colors::printSubSeparator(60);
        waitForInput();
    }
